...
#    numPoints = reactions['numPoints']
    
#    lengthX = [abs(nodes[elements[i][1]][0] - nodes[elements[i][0]][0])  for i in range(len(elements))]
#    lengthY = [abs(nodes[elements[i][1]][1] - nodes[elements[i][0]][1])  for i in range(len(elements))]
#    length  = [np.sqrt(lengthX[k]**2+lengthY[k]**2) for k in range(len(lengthX))]
    ...
    if quantity == 'Deformed Shape':
        reactions={'uh':displacements[loadgroup]['Displacement X'],
                   'uv':displacements[loadgroup]['Displacement Y']}
        ...
        if quantity in ['Normal Force','Shear Force','Moment']:
            reaction = internal_forces[loadgroup][quantity]
        elif quantity in ['Displacement X','Displacement Y','Rotation Z']:
            reaction = displacements[loadgroup][quantity]
...
#            ax.plot(xmin, ymin, color=minColor, alpha=0.4)
    ...
    if savePlot:
        ...
        plt.savefig('Plots '+title+'/Loadgroup '+str(loadgroup)+'_'+quantity+'.png',dpi=300, bbox_inches='tight')
